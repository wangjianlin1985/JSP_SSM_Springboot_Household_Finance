...
public class ExpendController extends BaseController {
	...
	public void userAdd(@Validated Expend expend, BindingResult br,
	...
	public void statistic(HttpServletRequest request,HttpServletResponse response) throws Exception {
    	...
    	for(int i=1;i<=12;i++) {
    		...
    		ArrayList<Expend> expendList = expendService.queryExpend(null, "", null, "", monthString, null);
    		for(Expend expend: expendList) {
    			monthMoney += expend.getExpendMoney();
    		}
    		xData.put(i+"月");
	...
	public String userFrontlist(@ModelAttribute("exprendTypeObj") ExpendType exprendTypeObj,String expendPurpose,@ModelAttribute("payWayObj") PayWay payWayObj,String payAccount,String expendDate,@ModelAttribute("userObj") UserInfo userObj,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "Expend/expend_userFrontquery_result"; 
	}
