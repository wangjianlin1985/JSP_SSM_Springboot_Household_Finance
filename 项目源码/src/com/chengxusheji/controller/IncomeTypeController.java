...
public class IncomeTypeController extends BaseController {
	...
	public void listAll(HttpServletResponse response) throws Exception {
		...
		for(IncomeType incomeType:incomeTypeList) {
			...
			jsonIncomeType.accumulate("typeId", incomeType.getTypeId());
			jsonIncomeType.accumulate("typeName", incomeType.getTypeName());
			jsonArray.put(jsonIncomeType);
		}
	...
	public String frontshow(@PathVariable Integer typeId,Model model,HttpServletRequest request) throws Exception {
		/*根据主键typeId获取IncomeType对象*/
        IncomeType incomeType = incomeTypeService.getIncomeType(typeId);

        request.setAttribute("incomeType",  incomeType);
        return "IncomeType/incomeType_frontshow";
	}

	/*ajax方式显示收入分类修改jsp视图页*/
	...
	public String delete(@PathVariable Integer typeId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  incomeTypeService.deleteIncomeType(typeId);
	            request.setAttribute("message", "收入分类删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "收入分类删除失败!");
				return "error";
	...
	public void delete(String typeIds,HttpServletRequest request,HttpServletResponse response) throws IOException, JSONException {
    	...
    	boolean success = false;
        try { 
        	int count = incomeTypeService.deleteIncomeTypes(typeIds);
        	success = true;
	...
	public void OutToExcel( Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        for(int i=0;i<incomeTypeList.size();i++) {
        	...
        	dataset.add(new String[]{incomeType.getTypeId() + "",incomeType.getTypeName()});
        }
        /*
