...
public class IncomeController extends BaseController {
	...
	public void userAdd(@Validated Income income, BindingResult br,
	...
	public void statistic(HttpServletRequest request,HttpServletResponse response) throws Exception {
    	...
    	for(int i=1;i<=12;i++) {
    		...
    		ArrayList<Income> incomeList = incomeService.queryIncome(null, "", null, "", monthString, null);
    		for(Income income: incomeList) {
    			monthMoney += income.getIncomeMoney();
    		}
    		xData.put(i+"月");
	...
	public String userFrontlist(@ModelAttribute("incomeTypeObj") IncomeType incomeTypeObj,String incomeFrom,@ModelAttribute("payWayObj") PayWay payWayObj,String payAccount,String incomeDate,@ModelAttribute("userObj") UserInfo userObj,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "Income/income_userFrontquery_result"; 
	}
